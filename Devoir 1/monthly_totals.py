...
def pivot_months_pandas(data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # rename columns so we dont get things mixed up
    grouped_data.rename(columns={'precipitation': 'total_precipitation', 'date': 'count'}, inplace=True)

    monthly, counts = grouped_data.pivot(index='name', columns='month', values='total_precipitation').fillna(0), grouped_data.pivot(index='name', columns='month', values='count').fillna(0)
    monthly = monthly.round(1) # round up at 1 decimal
...
def compute_pairwise(df: pd.DataFrame, func: callable) -> pd.DataFrame:
    ...
    **chaque paire de lignes du dataframe**.
    ...
    ```
    """
    condensed_matrix = pdist(df, metric=func)
    
    redundant_matrix = squareform(condensed_matrix)

    new_df = pd.DataFrame(redundant_matrix, index=df.index, columns=df.index)
...
def compute_pairwise_distances(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df = df.set_index('name')[['latitude', 'longitude']].drop_duplicates().sort_index()

    new_df = compute_pairwise(df, geodesic)
...
def correlation(u, v) -> float:
    ...
    # calculate the mean and standard deviation by filtering the rows first
    u_mean = np.mean(u[idx])
    v_mean = np.mean(v[idx])
    u_std = np.std(u[idx])
    v_std = np.std(v[idx])

    corr = np.mean((u[idx] - u_mean) * (v[idx] - v_mean)) / (u_std * v_std)
...
def compute_pairwise_correlation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    pivoted_df = df.pivot(index='name', columns='date', values='precipitation') # pivoting the df like in prev methods

    correlation_matrix = compute_pairwise(pivoted_df, correlation) # we pass the correlation method as the callable func
...
def compute_pairwise_correlation_pandas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    pivoted_df = df.pivot(index='date', columns='name', values='precipitation')
